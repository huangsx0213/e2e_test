...
const mockSqliteRunPersister = vi.hoisted(() => vi.fn(() => ({
  saveAgentLog: vi.fn(),
  updateRunStatus: vi.fn(),
  insertAuditLog: vi.fn(),
})));

vi.mock('../infrastructure/db/pipeline-repository.ts', () => ({
...
vi.mock('../../../../shared/ai/pipeline.ts', () => ({ createNlPipeline: mockCreateNlPipeline }));
vi.mock('../../../../shared/ai/prompt-version.ts', () => ({ computePromptVersion: mockComputePromptVersion }));
vi.mock('../pipeline-persister.ts', () => ({ RunPersister: {}, SqliteRunPersister: mockSqliteRunPersister }));
...
describe('startPipeline e2e', () => {
  ...
  beforeEach(() => {
    ...
    service = new PipelineService(sseGateway, 10);
  });
  ...
  describe('happy path', () => {
    it('completes a single-batch auto pipeline run', async () => {
      ...
      const persisterInstance = mockSqliteRunPersister.mock.results[0].value;
