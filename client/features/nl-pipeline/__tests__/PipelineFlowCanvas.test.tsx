...
describe('PipelineFlowCanvas', () => {
  ...
  it('TC-2.1: renders all 8 nodes in correct order', () => {
    render(React.createElement(PipelineFlowCanvas, defaultProps));
  ...
  it('TC-2.2: node shows running status with pulse class', () => {
    ...
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, nodes }));
  ...
  it('TC-2.3: selected node has ring highlight', () => {
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, selectedNodeId: 'agent_test_analyst' }));
    const agent = screen.getByText('Test Analyst').closest('[class*="border-2"]');
  ...
  it('TC-2.4: progress bar shows batch info', () => {
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, batch: 1, totalBatches: 3 }));
    expect(screen.getByText('Batch 1/3')).toBeTruthy();
  });

  it('TC-2.5: Abort button visible when running', () => {
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, isRunning: true }));
    expect(screen.getByText('Abort')).toBeTruthy();
  });

  it('TC-2.5: Abort button hidden when not running', () => {
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, isRunning: false }));
  ...
  it('TC-2.4: shows generated cases count', () => {
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, generatedCases: 28 }));
    expect(screen.getByText('28 cases')).toBeTruthy();
  });

  it('clicking a node triggers onNodeClick', () => {
    const onNodeClick = vi.fn();
    render(React.createElement(PipelineFlowCanvas, { ...defaultProps, onNodeClick }));
