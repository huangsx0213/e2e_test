...
vi.mock('@/shared/hooks/useQueryHooks', () => ({
  ...
  usePipelineRuns: vi.fn().mockReturnValue({ data: [], refetch: vi.fn() }),
...
function renderWithProviders(ui: React.ReactElement) {
  ...
  return render(
    React.createElement(PipelineRunDepsProvider as any, { deps: mockDeps },
...
describe('AiPipelinePage', () => {
  ...
  it('TC-4.1: shows project selection prompt when no projectId', () => {
    renderWithProviders(React.createElement(AiPipelinePage, { currentProjectId: null }));
    expect(screen.getByText('Select a project to continue')).toBeTruthy();
  });

  it('renders header with AI Pipeline title', () => {
    renderWithProviders(React.createElement(AiPipelinePage, defaultProps));
    expect(screen.getByText('AI Pipeline')).toBeTruthy();
  });
  ...
  it('shows header with AI Pipeline title and buttons', () => {
    renderWithProviders(React.createElement(AiPipelinePage, defaultProps));
