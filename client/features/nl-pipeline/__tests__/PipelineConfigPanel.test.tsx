...
describe('PipelineConfigPanel', () => {
  ...
  const defaultProps = {
    requirements: [],
    businessFlows: [],
    onStart: vi.fn() as (config: PipelineStartConfig) => void,
    disabled: false,
  };

  it('TC-1.1: renders requirement tree with expand/collapse', () => {
    const reqs = buildTreeSample();
    renderWithQuery(React.createElement(PipelineConfigPanel, { ...defaultProps, requirements: reqs }));
  ...
  it('TC-1.4: Business Flow only shows approved by default', () => {
    ...
    renderWithQuery(React.createElement(PipelineConfigPanel, { ...defaultProps, businessFlows: flows }));
  ...
  it('TC-1.6: mode toggle switches Auto/Interactive', () => {
    renderWithQuery(React.createElement(PipelineConfigPanel, { ...defaultProps }));
