...
import type { TestSuite, Project } from "@/shared/types";
...
export const SuiteExecutionRunner: React.FC<SuiteExecutionRunnerProps> = ({
  ...
  const {
    logs, status, queuePosition, progress, selectedEnv, selectedAgentId,
    ...
    <div className="flex flex-col h-full w-full bg-slate-900 overflow-hidden">
      <div className="h-16 px-6 bg-slate-900 border-b border-slate-800 flex justify-between items-center text-white shrink-0">
        <div className="flex items-center gap-4">
          <div
            className={`p-2 rounded-full ${isRunning ? "bg-blue-500/20 text-blue-400" : isQueued ? "bg-purple-500/20 text-purple-400" : status === "COMPLETED" ? "bg-emerald-500/20 text-emerald-400" : status === "IDLE" ? "bg-gray-500/20 text-gray-400" : "bg-red-500/20 text-red-400"}`}
          >
            {isRunning && <Loader2 className="animate-spin" size={20} />}
            {isQueued && <Layers className="animate-pulse" size={20} />}
            {status === "COMPLETED" && <CheckCircle2 size={20} />}
            {status === "FAILED" && <XCircle size={20} />}
            {status === "IDLE" && <PlayCircle size={20} />}
          </div>
          <div>
            <h3 className="font-semibold text-lg tracking-tight">Run Suite: {suite.name}</h3>
            <p className="text-xs text-slate-400 font-medium">
              {isQueued ? `Waiting in queue${queuePosition ? ` (Position: ${queuePosition})` : ''}…` : `${suite.cases.length} Cases`}
            </p>
          </div>
        </div>
