        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-2">
              ...
              across {environments.length} environments.
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <h3 className="font-semibold text-slate-900 flex items-center gap-2">
              <Activity size={18} className="text-slate-400" /> Recent Run
              Summary
            </h3>
            {latestReport && (
              <button
          ...
          {latestReport ? (
            <div
              className="p-6 flex flex-col md:flex-row items-center justify-between gap-6 cursor-pointer hover:bg-slate-50/50 transition-colors"
              onClick={() => onNavigate("REPORTS")}
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-lg font-bold text-slate-900">
                    {latestReport.suiteName || latestReport.suiteId}
                  </h4>
                  <div className="flex items-center gap-4 mt-1 text-sm text-slate-500">
                    <span className="flex items-center gap-1.5">
                      <Globe size={14} /> {latestReport.environment || "DEV"}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Clock size={14} /> {getTimeAgo(latestReport.startTime)}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Activity size={14} />{" "}
                      {formatDuration(
                        latestReport.startTime,
                        latestReport.endTime,
                      )}
                    </span>
                  </div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p
                    className={`text-xl font-bold ${
                      latestReport.passRate === 100
                        ? "text-emerald-600"
                        : latestReport.passRate >= 50
                          ? "text-amber-600"
                          : "text-red-600"
                    }`}
                  >
            ...
            <div className="p-12 flex flex-col items-center justify-center text-slate-400 gap-3">
              ...
              <p className="text-sm font-medium">
                No execution history found for this project
              </p>
              <button
                onClick={() => onNavigate("RUN")}
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-6 flex-1">
              ...
              <p className="text-slate-500 text-sm mb-6">
                Configure REST endpoints, headers, and request bodies for API
                testing.
              </p>
...
const NavCard = ({
  ...
  >
    <div className="p-6 flex-1">
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`w-12 h-12 rounded-lg ${bgClass} flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform`}
        >
          {icon}
        </div>
