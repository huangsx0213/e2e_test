...
> 合并自: `prd-requirements-pipeline.md`, `pipeline-istqb-design.md`, `ai-pipeline-e2e-test-plan.md`, `superpowers/specs/*`, `superpowers/plans/*`
...
3. **AI-Powered Automated Test Case Generation** — 将 NL 用例转为 QuantumQA Suite/Case/Step（后续阶段）

### 1.3 核心设计原则

- 每阶段支持 Human-in-the-Loop 审核和编辑
- 多 Agent 角色模拟，定义明确的输入/输出契约
- Provider-agnostic LLM 层（Azure OpenAI / Nvidia NIM / OpenRouter / OpenAI）
- 全链路追溯存储在 DB 中
- 复用现有 CRUD factory、module pattern 和 shared contract 约定

### 1.4 关键决策

- **No LangChain SDK** — 仅使用 `@langchain/langgraph` 做 StateGraph 编排，Provider 层用原生 `fetch()`
- **No RAG** — 使用 Skill-based 结构化索引 + 分批处理替代向量数据库
- **No separate AI microservice** — Pipeline 在现有 Node.js 进程内运行
...
```
shared/ai/                     # 核心 AI 框架
├── pipeline.ts                # LangGraph StateGraph (233 行)
├── agent.ts                   # Agent runtime: retry/timeout/cache/guard (305 行)
├── provider.ts                # LLM provider 适配器 + 熔断器 (281 行)
├── provider-types.ts          # ProviderConfig / CircuitBreakerState 类型
├── cache.ts                   # LLM 输出缓存 (SHA-256 + SQLite)
├── guard.ts                   # Prompt 注入检测
├── semaphore.ts               # 并发控制器
...
server/modules/ai-pipeline/    # Pipeline Orchestrator
├── index.ts                   # 纯路由层 (103 行)
├── pipeline-scope.ts          # PipelineExecutionScope: DB 写入 + SSE 发射 (182 行)
├── business-flow-blueprint.ts # BusinessFlow 转 blueprint
├── application/
│   └── pipeline-service.ts    # PipelineService: 编排/并发/runBatch (476 行)
├── infrastructure/
│   ├── db/
│   │   └── pipeline-repository.ts # PipelineRepository: DB 操作 (240 行)
│   └── sse/
│       ├── sse-gateway.ts     # SSEGateway: EventEmitter + SSE 流管理 (75 行)
│       └── __tests__/
├── __tests__/                 # 测试

client/features/nl-pipeline/   # 前端 Pipeline 页面
├── AiPipelinePage.tsx         # 主编排页面
├── PipelineConfigPanel.tsx    # 左侧配置面板
├── PipelineFlowCanvas.tsx     # 中间流程图
...
interface TestAnalystOutput {
  ...
  testConditions: TestCondition[];  // 原子化测试条件
}
...
**输入**: `approved TestCondition[]` + 项目上下文
...
4. Cover positive/negative/boundary/error — 每个 TestCondition 至少 1 正 + 1 反
5. Repeatable — 不依赖其他用例的执行结果
6. Data is specific — 使用具体值
...
| API 接口参数 | EP + BVA | 每个参数的有效/无效/边界值 |
...
| Skill | 提供给 Role 的 ISTQB 知识 | 被谁使用 |
|---|---|---|
| `requirement-index` | 需求索引 JSON，轻量浏览需求全景 | TestAnalyst |
| `requirement-query` | "如何按 tag/level/priority 查询需求子集" | TestAnalyst |
| `requirement-analysis` | ISTQB 分析检查表 + 需求特征→技术映射 | TestAnalyst |
| `test-analyst` | 技术选择规则 + Condition 分类标准 | TestAnalyst |
| `test-designer` | 用例设计标准 ISTQB + 自审维度 | TestDesigner |
| `quality-manager` | 6 维质量审阅标准 + 问题分级 | QualityManager |
...
shared/ai/skills/
  requirement-index/
    ...
    references/index.json        # 自动生成的索引 [{ id, title, level, summary(≤200字), tags, priority, ... }]
  requirement-query/
...
**工作机制**:
1. Orchestrator 读取 `requirement-index/references/index.json` 了解全量需求
2. 按 epic 分组，每批只处理一个 epic + 其子节点
3. Agent 1 每批接收一个子集，处理完后返回 conditions
4. Orchestrator 跨批执行去重和交叉校验
5. 索引文件在需求变更时自动重建
...
条件边：
- `checkpoint_1`: phase=analysis → 回 `agent_test_analyst`（retry），否则进 `agent_test_designer`
- `checkpoint_2`: phase=design → 回 `agent_test_designer`（retry），否则进 `agent_quality_manager`
- `checkpoint_3`: phase=quality → 回 `agent_quality_manager`（retry），否则到 END
...
```
1. buildRequirementIndex(projectId) → index (按 epic 分组)
...
4. for each batch:
   ...
   e. Agent 节点执行:
      - callback → scope.recordAgentStart() → SSE: agent:start + DB INSERT pipeline_agent_logs
      - callback → scope.recordAgentStep() → SSE: agent:step (rawTrace 累积)
      - callback → scope.recordAgentThinking() → SSE: agent:thinking
      - callback → scope.recordAgentComplete() → SSE: agent:complete + DB UPDATE pipeline_agent_logs
   ...
   i. pipeline 继续或重试
5. 跨批合并: 去重 + 一致性检查 + 聚合 coverageMatrix
6. 保存到 DB → SSE: pipeline:complete (scope.markComplete)
```

所有 SSE 事件最终通过 `SSEGateway.emit(runId, event, data)` 发出。
...
|---|---|---|
| `heartbeat` | `{ ts }` | 每 15s |
| `phase:start` | `{ phase, message }` | Pipeline 阶段开始 |
| `pipeline:context` | `{ flows, indexEntries }` | 上下文准备完成 |
| `pipeline:budget` | `{ estimated, limit, warning, message }` | Token 预算估计 |
| `agent:start` | `{ agentName, batch, timestamp }` | Agent 开始执行 |
...
后端的 `SSEGateway` 类（`infrastructure/sse/sse-gateway.ts`）统一管理：
- `emitters` Map：每个 runId 一个 EventEmitter
- `emit(runId, event, data)`：广播事件给所有该 runId 的订阅者
- `attachStream(runId, res)`：绑定 SSE 响应流，自动注册心跳（15s）和 `pipeline:complete`/`pipeline:error` 自动关闭
- `cleanup(runId)`：清理 emitter 和 流，在 `PipelineService` 的 `finally` 块或 `deleteRun` 时调用

`PipelineExecutionScope`（`pipeline-scope.ts`）通过注入的 emit 函数直接发射事件：
```typescript
...
| `POST` | `/api/pipeline/:runId/resume` | 恢复停止的 Pipeline |
...
```typescript
{
  requirementIds: string[];       // 选中的需求 ID
  providerConfigName: string;     // AI Provider 配置名称
  mode: 'auto' | 'interactive';  // 运行模式
  flowIds?: string[];            // 选中的 Business Flow ID（仅存于 config）
...
**处理流程**:
1. 路由层创建 runId、插入 DB（含 config）、`res.json({ runId })`
2. 异步调用 `PipelineService.startPipeline(runId, projectId, { requirementIds, providerConfigName, mode })`

**模式差异**:
- **Auto**: `PipelineService.runBatch()` 自动通过所有 checkpoint，并行度由 `Semaphore(MAX_CONCURRENT)` 控制
- **Interactive**: `PipelineService.runBatch()` 在每个 checkpoint 暂停（`Promise` 队列），等待 `resumeRun()`
...
// Pipeline 在 checkpoint 暂停（runBatch 内）:
...
CREATE TABLE IF NOT EXISTS pipeline_runs (
  ...
  checkpoint_data TEXT,                               -- JSON: 当前 checkpoint 的 payload
  ...
  token_limit     INTEGER,                            -- 该 run 的 token 上限
...
- `usePipelineSSE` Hook: 使用 `fetch()` + `ReadableStream` 消费 SSE（替代 EventSource，因 POST 请求需要传递配置）
...
- `runAgent()` 中所有 `provider.chat()` 调用使用 `AbortController`
...
| **Rate-limit** | `includes('429')` | 延迟加倍（`delay * 2`），最多 3 次 |
| **Transient** | `fetch failed` / `ECONNRESET` / `socket hang up` | 标准退避重试 |
| **Validation** | `SyntaxError` / `ZodError` | 重试并注入错误提示到对话上下文 |
| **Non-retryable** | 其他 | 立即抛出 |

### 10.3 Provider Fallback + 熔断器

```typescript
circuitBreaker: {
  failureThreshold: number;   // 默认 5 次
  resetTimeoutMs: number;     // 默认 60,000ms
}
```

- 主 provider 连续失败 → 切换 fallback
- 全部 fallback 失败 → 抛出 `AllProvidersFailedError`
- 熔断期间定时放行一次探测请求（half-open）

### 10.4 并发控制

- 全局 `PipelineService` 内 `acquireSlot()`/`releaseSlot()`，默认 `MAX_CONCURRENT = 3`
- 队列等待：超出排队（`runQueue`）
- 批次内并行：Auto 模式下各 epic 用 `Semaphore` 控制并行度
...
- TTL: 24 小时
- 缓存表 `agent_cache`，按 `promptVersion` 批量失效
- 通过 `useCacheStore()` 注入（`PipelineService` 构造函数调用），当前使用 `PipelineRepository.getCacheStore()`
...
- Run 前预算预检查（按需求数量 × 平均 1000 tokens）
- RSA 后实际 tracking

### 10.9 Audit Log

每个 checkpoint 操作写入 `pipeline_audit_log`：
...
| TC-1.7 | Start 按钮在无选择时禁用 | P0 |
| TC-1.8 | Start 按钮在有选择时可用 | P0 |
...
| TC-2.3 | 选中节点高亮 | P1 |
| TC-2.4 | 进度条显示 batch 信息 | P0 |
| TC-2.5 | 运行中显示 Abort 按钮 | P0 |

### 11.3 Node Detail

| # | 测试用例 | 优先级 |
|---|---|---|
| TC-3.1 | 未选中节点显示占位符 | P0 |
| TC-3.2 | Agent 节点显示 4 个 Tab: input/output/trace/errors | P0 |
...
| TC-4.3 | SSE 处理所有事件 | P0 |
| TC-4.4 | Abort 确认弹窗 | P0 |
| TC-4.5 | 开始 Pipeline 重置状态 | P0 |
...
| # | 任务 | 交付物 |
|---|---|---|
| 1 | 创建 3 个 Role + Zod schemas | `shared/ai/roles/` |
| 2 | 扩充 SKILL.md 内容 | 各 Skill 目录 |
| 3 | Index generator + 需求变更自动重建 | `index-generator.ts`, `repository.ts` |
| 4 | Pipeline rewrite: interrupt() + ROPer Agent 调用 | `pipeline.ts` |
| 5 | SSE endpoint: 调 createNlPipeline() + 处理 interrupt() | `ai-pipeline/index.ts` |
| 6 | 超时控制: AbortController + 60s timeout | `agent.ts` + `provider.ts` |
| 7 | 指数退避重试: 2s/4s/8s + 错误类型区分 | `agent.ts` |
| 8 | 并发控制: Semaphore + 队列 | `semaphore.ts` |
| 9 | API Key 加密: AES-256-GCM | `provider_configs` 表 |
| 10 | SSE 心跳: 15s heartbeat | SSE endpoint |
| 11 | DB migration: pipeline_runs + provider_configs | migrations |
...
| 16 | Callback 扩展传递 inputPrompt + outputData | `shared/ai/pipeline.ts` |
| 17 | 前端 Input/Trace Tab: 显示 ChatMessage[] + TraceEntry 时间戳 | `PipelineNodeDetail.tsx` |
| 18 | SSEGateway: 提取 EventEmitter 管理为独立类 | `infrastructure/sse/sse-gateway.ts` |
| 19 | PipelineRepository: DB 操作提取为 repository | `infrastructure/db/pipeline-repository.ts` |
| 20 | PipelineService: 编排/并发/resumeWaiters 提取 | `application/pipeline-service.ts` |
| 21 | 路由层精简: index.ts 866 → 103 行 | `index.ts` |
...
| 24 | 审计日志: pipeline_audit_log + checkpoint 快照 | DB + API |
| 25 | Model/Prompt 版本追踪 | `prompt_version`, `model_name` |
| 26 | Cost Cap: 项目级限额 + run 级上限 | Orchestrator + API |
| 27 | LLM Output 缓存: agent_cache 表 + TTL | `cache.ts` + `agent.ts` |

### Phase 4 — 迭代优化

| # | 任务 | 交付物 |
|---|---|---|
| 28 | 并行批处理: Semaphore 控制并行度 | Orchestrator |
| 29 | 细粒度进度事件: agent:progress | Agent runtime + SSE |
| 30 | 测试策略完善: Snapshot + LLM-as-judge + Chaos | `__tests__/` |
| 31 | Token 预算预检查 | 运行前估算 |
...
| 34 | 批处理 case 级进度 | Orchestrator + SSE |
